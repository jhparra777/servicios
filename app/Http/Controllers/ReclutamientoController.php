...
use App\Models\Nivel5;
use App\Models\Nivel6;
use App\Models\Nivel7;
use App\Models\Nivel8;
...
class ReclutamientoController extends Controller
{
    ...
    public function gestion_req($req_id, Request $data)
    {
        ...
        if(route("home") == "http://komatsu.t3rsc.co" || route("home") == "https://komatsu.t3rsc.co"){
            if(!empty($requermiento->solicitud_id)){
                $solicitud=Solicitudes::find($requermiento->solicitud_id);
            }
        }
        ...
        /*
            foreach($preguntas_req as $key => $preg){
                $respuestas_user = PregReqResp::join('req_preguntas', 'req_preguntas.id', '=', 'preg_req_resp.preg_req_id')
                ->join('users', 'users.id', '=', 'preg_req_resp.user_id')
                ->where('preg_req_resp.preg_req_id', $preg->req_preg_id)
                ->select(
                    'preg_req_resp.*',
        ...
        */
        ...
        ->select(
            ...
            DB::raw('(select apto from procesos_candidato_req where procesos_candidato_req.requerimiento_id = requerimiento_cantidato.requerimiento_id AND procesos_candidato_req.candidato_id = requerimiento_cantidato.candidato_id AND procesos_candidato_req.proceso = \'ENVIO_APROBAR_CLIENTE\') apto')
        )
        ...
        if(route("home") == "http://komatsu.t3rsc.co" || route("home") == "https://komatsu.t3rsc.co"){
            ...
            ['requerimiento_id' => $req_id,'status' =>1 ])
            ->select('*')
            ->get();
        }else{
            $candidatos_postulados = CandidatosFuentes::where(
    ...
    */
        ...
        if($data->lleva_ordenes == 'si' && !$data->has('omnisalud_opcion')) {
            ...
            $candidato = ReqCandidato::join("datos_basicos", "datos_basicos.user_id", "=", "requerimiento_cantidato.candidato_id")
                ...
                ->select(
                    ...
                    "empresa_logos.nombre_empresa as sociedad"
                )
            ->first();
    ...
    public function pdf_hv($user_id, Request $data)
    {
        ...
        if ($sitio->nueva_hv == 'enabled') { 
            $footerHtml = view()->make('cv.includes.footer_proservis')->render();
            $pdf = \SnappyPDF::loadView('cv.pdf_hv_nuevo', compact(
                'idiomas',
                'sitio',
                'experienciaActual',
            ...
            $pdf = \SnappyPDF::loadView('cv.pdf_hv',[
    ...
    public function enviar_contratar2(Request $data)
    {
        ...
        $candidato = ReqCandidato::join("datos_basicos", "datos_basicos.user_id", "=", "requerimiento_cantidato.candidato_id")
            ...
            ->select(
                ...
                "requerimiento_cantidato.id as req_candidato",
                "requerimiento_cantidato.requerimiento_id as req",
                "requerimiento_cantidato.fecha_tentativa_ingreso as fecha_ingreso"
            )
        ->first();
        ...
        if ($candidato != null) {
            ...
            $centros_costos = ["" => "Seleccionar"] + CentroCostoProduccion::where("cod_division", $data->cliente_id)->where("active", 1)->orderBy('descripcion')->pluck("descripcion", "id")->toArray();
            ...
            $caja_compensaciones = ["" => "Seleccionar"] + CajaCompensacion::where("active", 1)->orderBy('descripcion')->pluck("descripcion", "id")->toArray();
            ...
            $formas_pago = ["" => "Seleccionar"] + FormaPago::orderBy('descripcion')->pluck("descripcion", "id")->toArray();

            $nivel_5 = ["" => "Seleccionar"] + Nivel5::join("clientes_nivel_5", "clientes_nivel_5.nivel_5_id", "=", "nivel_5.id")
                                                ->where("clientes_nivel_5.cliente_id", $data->cliente_id)
                                                ->pluck("nivel_5.descripcion", "nivel_5.id")
                                                ->toArray();
            
            $nivel_6 = ["" => "Seleccionar"] + Nivel6::join("clientes_nivel_6", "clientes_nivel_6.nivel_6_id", "=", "nivel_6.id")
                                                ->where("clientes_nivel_6.cliente_id", $data->cliente_id)
                                                ->pluck("nivel_6.descripcion", "nivel_6.id")
                                                ->toArray();
            $nivel_7 = ["" => "Seleccionar"] + Nivel7::join("clientes_nivel_7", "clientes_nivel_7.nivel_7_id", "=", "nivel_7.id")
                                                ->where("clientes_nivel_7.cliente_id", $data->cliente_id)
                                                ->pluck("nivel_7.descripcion", "nivel_7.id")
                                                ->toArray();
            $nivel_8 = ["" => "Seleccionar"] + Nivel8::join("clientes_nivel_8", "clientes_nivel_8.nivel_8_id", "=", "nivel_8.id")
                                                ->where("clientes_nivel_8.cliente_id", $data->cliente_id)
                                                ->pluck("nivel_8.descripcion", "nivel_8.id")
                                                ->toArray();

            $nominas_clientes = [""=>"Seleccionar"] + DB::table("nominas_clientes")->where("cliente_id", $data->cliente_id)->where("active", 1)->pluck("descripcion","id")->toArray();

            $objetos_contrato = [""=>"Seleccionar"] + DB::table("objetos_contrato")->where("active", 1)->pluck("descripcion","id")->toArray();
            
            $subtipo_cotizante = [""=>"Seleccionar"] + DB::table("subtipo_cotizante")->where("active", 1)->pluck("descripcion","id")->toArray();

            $clases_riesgo =  ["" => "Seleccionar"] + CentroTrabajo::get()->pluck("nombre_ctra", "id")->toArray();
        ...
        $tipos_ingreso = [""=>"Seleccionar"] + DB::table("tipos_ingreso")->where("active", 1)->pluck("descripcion","id")->toArray();

        $tipos_empleados = ["" => "Seleccionar"] + TipoEmpleado::where("active", 1)->orderBy("descripcion")->pluck("descripcion", "id")->toArray();

        $agencias =  ["" => "Seleccionar"] + AgenciaUsuario::join("agencias", "agencias.id", "=", "agencia_usuario.id_agencia")
        ->where('agencia_usuario.id_usuario', $this->user->id)
        ->pluck("agencias.descripcion",
                 "agencias.id")->toArray();
    ...
    public function enviar_contratar_req(Request $data)
    {
        ...
        if(route("home") == "http://demo.t3rsc.co" || route("home") == "https://demo.t3rsc.co" ||
            ...
            $caja_compensaciones = ["" => "Seleccionar"] + CajaCompensacion::where("active", 1)->pluck("descripcion", "id")->toArray();
    ...
    */
        ...
        try {
            ...
            if ($se_puede_contratar == 'si') {
                ...
                if($sitio->asistente_contratacion == 1){
                    ...
                    $historialContratacion->nivel_5_id = $data->nivel_5_id;
                    $historialContratacion->nivel_6_id = $data->nivel_6_id;
                    $historialContratacion->nivel_7_id = $data->nivel_7_id;
                    $historialContratacion->nivel_8_id = $data->nivel_8_id;
                    $historialContratacion->centro_trabajo_id = $data->clase_riesgo;
                    ...
                    $historialContratacion->forma_pago_id   = $data->forma_pago;
                    ...
                    $historialContratacion->nomina_cliente_id = $data->nomina_cliente_id;
                    $historialContratacion->objeto_contrato_id = $data->objeto_contrato_id;
                    $historialContratacion->subtipo_cotizante_id = $data->subtipo_cotizante_id;
                    $historialContratacion->tipo_empleado_id = $data->tipo_empleado_id;
                    $historialContratacion->salario = $data->salario;
                    $historialContratacion->centro_trabajo = $data->centro_trabajo;
                ...
                */
                ...
                $request4 = new Request(["user_id"=>$requerimiento_cand->candidato,"download"=>1]);

                $controll = new ContratacionController();
                $aceptacion = $controll->aceptacionPoliticaTratamientoDatos($request4);
                Storage::disk('public')->put('documentos_candidatos/'.$requerimiento_cand->cedula.'/'.$requerimiento_cand->requerimiento_id.'/seleccion/aceptacion_politicas_datos.pdf',$aceptacion);
            //Fin si se contrata
            } else {
    ...
    public function editar_informacion_contratacion(Request $data)
    {
        ...
        $nivel_5 = ["" => "Seleccionar"] + Nivel5::join("clientes_nivel_5", "clientes_nivel_5.nivel_5_id", "=", "nivel_5.id")
                                            ->where("clientes_nivel_5.cliente_id", $data->cliente)
                                            ->pluck("nivel_5.descripcion", "nivel_5.id")
                                            ->toArray();
        
        $nivel_6 = ["" => "Seleccionar"] + Nivel6::join("clientes_nivel_6", "clientes_nivel_6.nivel_6_id", "=", "nivel_6.id")
                                            ->where("clientes_nivel_6.cliente_id", $data->cliente)
                                            ->pluck("nivel_6.descripcion", "nivel_6.id")
                                            ->toArray();
        $nivel_7 = ["" => "Seleccionar"] + Nivel7::join("clientes_nivel_7", "clientes_nivel_7.nivel_7_id", "=", "nivel_7.id")
                                            ->where("clientes_nivel_7.cliente_id", $data->cliente)
                                            ->pluck("nivel_7.descripcion", "nivel_7.id")
                                            ->toArray();
        $nivel_8 = ["" => "Seleccionar"] + Nivel8::join("clientes_nivel_8", "clientes_nivel_8.nivel_8_id", "=", "nivel_8.id")
                                            ->where("clientes_nivel_8.cliente_id", $data->cliente)
                                            ->pluck("nivel_8.descripcion", "nivel_8.id")
                                            ->toArray();

        $nominas_clientes = [""=>"Seleccionar"] + DB::table("nominas_clientes")->where("cliente_id", $data->cliente)->where("active", 1)->pluck("descripcion","id")->toArray();
        ...
        ->select(
            ...
            "requerimiento_contrato_candidato.centro_costo_id as centro_costos",
            "requerimiento_contrato_candidato.nivel_5_id",
            "requerimiento_contrato_candidato.nivel_6_id",
            "requerimiento_contrato_candidato.nivel_7_id",
            "requerimiento_contrato_candidato.nivel_8_id",
            "requerimiento_contrato_candidato.centro_trabajo_id",
            "requerimiento_contrato_candidato.forma_pago_id",
            "requerimiento_contrato_candidato.nomina_cliente_id",
            "requerimiento_contrato_candidato.objeto_contrato_id",
            "requerimiento_contrato_candidato.subtipo_cotizante_id",
            "requerimiento_contrato_candidato.tipo_empleado_id",
            "requerimiento_contrato_candidato.salario",
            "requerimiento_contrato_candidato.eps_id",
            "requerimiento_contrato_candidato.fondo_pensiones_id",
            "requerimiento_contrato_candidato.fondo_cesantia_id",
            "requerimiento_contrato_candidato.caja_compensacion_id",

            "requerimiento_cantidato.auxilio_transporte",
            "requerimiento_cantidato.tipo_ingreso",
            "requerimiento_contrato_candidato.fecha_ingreso as fecha_ingreso"
        )
        ->groupBy('procesos_candidato_req.candidato_id')
        ->orderBy('requerimiento_cantidato.id', 'DESC')
        ->first();
    ...
    public function guardar_informacion_contratacion(Request $data)
    {
        ...
        $actualizar_contrato->nivel_5_id            = $data->nivel_5_id;
        $actualizar_contrato->nivel_6_id            = $data->nivel_6_id;
        $actualizar_contrato->nivel_7_id            = $data->nivel_7_id;
        $actualizar_contrato->nivel_8_id            = $data->nivel_8_id;
        $actualizar_contrato->centro_trabajo_id     = $data->clase_riesgo;
        $actualizar_contrato->tipo_empleado_id      = $data->tipo_empleado_id;
        $actualizar_contrato->forma_pago_id         = $data->forma_pago;
        $actualizar_contrato->nomina_cliente_id     = $data->nomina_cliente_id;
        $actualizar_contrato->objeto_contrato_id    = $data->objeto_contrato_id;
        $actualizar_contrato->subtipo_cotizante_id  = $data->subtipo_cotizante_id;
    ...
    */
         ...
         try {
            ...
            foreach(json_decode($data->candidato_req) as $key => $req_candi_id) {
                ...
                if ($proceso_sst != null) {
                    ...
                    $mailTemplate = 2; //Plantilla con botón e imagen de fondo
                    $mailConfiguration = 1; //Id de la configuración
                    $mailTitle = "Evaluación inducción"; //Titulo o tema del correo

                    $nombre_completo = $candidato->nombres.' '. $candidato->primer_apellido.' '.$candidato->segundo_apellido;

                    $message_wp = $sitio->mensajePruebasWhatsapp($nombre_completo, $configuracion_sst->titulo_prueba, route('realizar_evaluacion_induccion_sst', ['req_id' => $candidato->req_id]));

                    event(new \App\Events\NotificationWhatsappEvent('57'.$candidato->telefono_movil,$message_wp));

                    //Cuerpo con html y comillas dobles para las variables
                    $mailBody = '
                        Hola '.$nombre_completo.', te informamos que nuestro analista quién está llevando a cabo tu proceso de selección te ha solicitado ver el siguiente <a href="'.asset("Proservis_video_sst.mp4").'" target="_blank"><b>video de capacitación</b></a>, revisar el archivo adjunto y responder las preguntas de la Evaluación Inducción que preparamos para ti.
                        <br/><br/>
                        Por favor haz clic en el botón “Realizar Evaluación Inducción”, ingresa a la plataforma usando tu número de identificación como usuario (sin puntos, comas ni signos) y también como contraseña.
                        ';
                    ...
                    Mail::send($triEmail->view, ['data' => $triEmail->data], function($message) use($candidato) {
                        $message->to([$candidato->email])
                        ->subject("Evaluación inducción")
                        ->attach(public_path('INDUCCION_EVALUACIÓN_SST.pdf'))
                        //->attach(public_path('6.Preservis_Video_6.mp4'))
    ...
    public function generar_carnet_general($id,$download=0)
    {
        ...
        $view = view('home.carnet_candidato_general', ['candidato'=>$candidato, 'req'=>$req, 'contrato'=>$contrato, 'empresa'=>$empresa, 'qrcode'=>$qrcode]);
        $pdf =  app('dompdf.wrapper');
        $pdf->loadHTML($view);
