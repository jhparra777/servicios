...
class HomeController extends Controller
{
    ...
    {   
        ...
        $reqcandidato = ReqCandidato::join("requerimientos", "requerimientos.id", "=", "requerimiento_cantidato.requerimiento_id")
                ...
                ->where("requerimiento_cantidato.candidato_id", 4)
                ->where("requerimiento_cantidato.requerimiento_id", 51)
            ...
            $req_contrato_candidato = RequerimientoContratoCandidato::leftJoin("entidades_afp", "entidades_afp.id", "=", "requerimiento_contrato_candidato.fondo_pensiones_id")
                ...
                ->select(
                    ...
                    "entidades_eps.descripcion as entidades_eps_des")
                ->where("requerimiento_contrato_candidato.candidato_id", 4)
                ->where("requerimiento_contrato_candidato.requerimiento_id", 51)
                ->first();

            $empresa_logo = DB::table("empresa_logos")->where('id', $reqcandidato->empresa_contrata)->first();

            $view = \View::make('home.pdf-carta-presentacion', [
                'candidato'     => $candidato,
            ...
            $pdf->loadHTML($view);
            return $pdf->stream('medica.pdf');

            return \SnappyPDF::loadView('home.pdf-carta-presentacion', [
                'candidato'     => $candidato,
                'reqcandidato'  => $reqcandidato,
                'empresa_logo'  => $empresa_logo,
                'req_contrato_candidato' => $req_contrato_candidato
        ...
        $requerimiento = Requerimiento::leftJoin('empresa_logos', "empresa_logos.id", "=", "requerimientos.empresa_contrata")
            ...
            ->where('requerimientos.id', 361)->first();

        //para habilitar el logo en el PDF
        $isPDF = true;

        $view = \View::make($vista, compact(
                "firma",
                "fecha",
                "user",
                "requerimiento",
    ...
    */
    ...
    public function empleos(Request $data)
    {
        ...
        ->whereRaw(DB::raw("requerimientos.id not in (select req_id from estados_requerimiento where estado = " . config('conf_aplicacion.C_TERMINADO') . ")"))
        //->groupBy("departamentos.cod_departamento","requerimientos.departamento_id")
        //->groupBy("ciudad.cod_ciudad", "requerimientos.ciudad_id")
        ->groupBy("ciudad.id")
        ...
        ->where(function ($where) use ($data) {
            ...
            if ($data->get("ciudad_id") != "") {
                $where->whereIn("ciudad.nombre", $data->get("ciudad_id"));
            }
    ...
    public function responder_preguntas_filtro($req_id, $cargo_id)
    {
        if(Sentinel::check()){
            ...
            $respuesta_filtro = PregReqResp::join('preguntas', 'preguntas.id', '=', 'preg_req_resp.preg_id')
            ->where('preguntas.filtro', 1)
            ->pluck('preguntas.id')
            ->toArray();

            $pregunta = Pregunta::join('tipo_pregunta', 'tipo_pregunta.id', '=', 'preguntas.tipo_id')
            ...
            ->where('preguntas.filtro', 1)
            ->whereNotIn('preguntas.id', $respuesta_filtro)
            ->select(
                'preguntas.*',
        ...
        foreach($data->respuesta_filtro as $key => $respuesta_id) {
            ...
            if($respuestas->minimo == 1){
                ...
                if ($total_preguntas_filtro > 0) {
                    ...
                    ->select('preg_req_resp.*', 'preguntas.tipo_id', 'preguntas.filtro')
                    ->where('preguntas.filtro', 1)
                    ...
                    if ($respuestas_preguntas < $total_preguntas_filtro) {
                    ...
                    if ($sitioModulo->proceso_automatico == 'enabled') {
                        $requerimiento = Requerimiento::select('solicitado_por', 'tipo_proceso_id')->find($req_id);
                        if ($sitioModulo->id_proceso_automatico == $requerimiento->tipo_proceso_id) {
                            ...
                            $campos_data = [
                                ...
                                'usuario_envio'              => $requerimiento->solicitado_por,
                            ...
                            event(new \App\Events\NotificationWhatsappEvent("57".$datos_candidato->telefono_movil,$mensaje_wpp));
                            ...
                            */
                            $mailTemplate = 2; //Plantilla con botón e imagen de fondo
                            $mailConfiguration = 1; //Id de la configuración
                            $mailTitle = "Prueba Ethical Values"; //Titulo o tema del correo

                            //Cuerpo con html y comillas dobles para las variables
                            $mailBody = "
                                Hola $datos_candidato->nombre_completo, has sido enviado/a en tu proceso de selección a realizar nuestra Prueba Ethical Values. <br>
    ...
    */
    ...
    public function informacionTrabajador($id)
    {
        ...
        ->select(
            ...
            "empresa_logos.nit as nit_sociedad"
        )
        ->groupBy('requerimientos.id')
        ->find($candidato->req);

        $requerimientoContratoCandidato = RequerimientoContratoCandidato::leftJoin("entidades_eps", "entidades_eps.id", "=", "requerimiento_contrato_candidato.eps_id")
            ->where('requerimiento_contrato_candidato.requerimiento_id', $candidato->req)
            ->where('requerimiento_contrato_candidato.candidato_id', $id)
            ->select(
