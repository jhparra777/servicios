...
class ClienteContratadosController extends Controller
{
    ...
    public function documentos_seleccion($candidato, $req, $req_can)
    {
        ...
        foreach ($tipo_documento as $key => &$tipo_doc) {
            $tipo_doc->documentos = DB::table('documentos')->select('nombre_archivo as nombre')->where('user_id', $candidato_id)->where('tipo_documento_id', $tipo_doc->id)->where("active",1)->latest()->limit(5)->get();
        }
        unset($tipo_doc);
