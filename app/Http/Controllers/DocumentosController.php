...
class DocumentosController extends Controller
{
    ...
    public function lista_documentos_medicos(Request $data)
    {
        ...
        return view("admin.reclutamiento.documentos_medicos", compact("candidatos"));
    }
    ...
    {
        session(["url_previa" => url(\URL::previous())]);
        $sitio_modulo = SitioModulo::first();
    ...
    public function nuevo_documento_medico_verificado(Request $data)
    {
        ...
        if($sitio_modulo->salud_ocupacional != 'si') {
            ...
            $f = TipoDocumento::where("active", "1")->where("descripcion", "EXÁMENES MÉDICOS")->first();
    ...
    {
       //dd($data->all());
       if(route("home")=="http://localhost:8000" || route("home")=="https://demo.t3rsc.co" || route("home")=="https://listos.t3rsc.co" || route("home")=="https://desarrollo.t3rsc.co" || route("home")=="https://proservis.t3rsc.co"){
     //dd($data->resultado);
         $tipoDocumento = ["" => "Seleccionar"] + TipoDocumento::where("active", "1")->pluck("descripcion", "id")->toArray();
    ...
    public function guardar_examen_medico(Request $data, Requests\DocumentoMedicoNuevoRequest $valida)
    {
        ...
        if($sitio_modulo->salud_ocupacional != "si") {
            ...
            $cant_archivos = count($data->file('archivo_documento'));
            if ($data->hasFile('archivo_documento')) {
                $cant_no_procesados = $this->guarda_exemenes_medicos_multiples($sitio_modulo->salud_ocupacional, $candidato, $data, $tipo->id, $descripcion, $proceso->requerimiento_id);
            }
            ...
            return response()->json(["success" => true,"ruta"=>$ruta, "mensaje"=> $mensaje]);

        } else {
            $orden = OrdenMedica::find($data->get("orden"));

            $ruta = route("admin.salud_ocupacional");
            ...
            if($data->get("resultado") == 3 || $data->get("resultado") == 4){
                ...
                $agencia = User::join('users_x_clientes','users_x_clientes.user_id','=','users.id')
                    ...
                    ->where('requerimientos.id', $req_can->requerimiento_id)
                    ->select("ciudad.agencia as agencia", "clientes.id as cliente")
                ->first();

                $emails = AgenciaUsuario::select("users.email as email", "users.name as nombre")
                    ...
                    ->whereIn("role_users.role_id", [23])
                    ->where("clientes.id", $agencia->cliente)
                    ->groupBy("users.id")
                ->get();
                ...
                $mailTitle = "Solicitud para la gestión de recomendaciones o restricciones médicas"; //Titulo o tema del correo
                ...
                foreach($emails as $key => $value) {
                    ...
                    $mailBody = "
                                {$value->nombre}, te informamos que el analista de selección que está llevando a cabo el proceso 
                                en el requerimiento No. {$req_can->requerimiento_id} para el cargo {$requerimiento->cargo} te ha enviado al candidato 
                                {$candidato->nombres} {$candidato->primer_apellido} {$candidato->segundo_apellido} para que gestiones las 
                                recomendaciones o restricciones del concepto médico. Por favor ingresa por el siguiente botón para realizar la gestión.
                    ";

                    $triEmail = triPostmaster::makeEmail($mailTemplate, $mailConfiguration, $mailTitle, $mailBody, $mailButton);

                    Mail::send($triEmail->view, ['data' => $triEmail->data], function($message) use($value) {

                        $message->to($value->email)
                            ->subject("Solicitud para la gestión de recomendaciones o restricciones médicas")
                            ->getHeaders()
            ...
            $cant_archivos = count($data->file('archivo_documento'));
            if ($data->hasFile('archivo_documento')) {
                $cant_no_procesados = $this->guarda_exemenes_medicos_multiples($sitio_modulo->salud_ocupacional, $candidato, $data, $tipo->id, "Resultado de examen medico", $req_can->requerimiento_id, $orden, $NuevoResultado);
            }
    ...
    public function tiene_vencimiento(Request $data)
    {
        $document=TipoDocumento::find($data->get("tipo_documento_id"));
        if($document->tiene_vencimiento == 1)
            return response()->json(["success"=>true, "tiene_vencimiento"=> true]);
        else
            return response()->json(["success"=>true, "tiene_vencimiento"=> false]);
    }    

    public function guarda_exemenes_medicos_multiples(string $salud_ocupacional, object $candidato, Request $data, int $tipo_documento, string $tipo_documento_descripcion, int $req_id, $orden = NULL, $NuevoResultado = NULL)
    {
        $cant_archivos = 0;
        $cant_no_procesados = 0;
        //$Documentos = collect([]);
        ...
        foreach ($data->file('archivo_documento') as $key => $imagen) {
            ...
            if($extension == "jpg" || $extension == "png" || $extension == "jpeg" || $extension == "pdf" || $extension == "PDF") {
                ...
                $documentos = new Documentos();
                if ($salud_ocupacional == 'si') {

                    $documentos->fill($data->all() + ["user_id" => $candidato->user_id, "numero_id" => $candidato->numero_id,"gestiono"=>$this->user->id]);
                    $documentos->save();

                    $name_documento = "documento_verificados_" . $documentos->id . "." . $extension;
                    $imagen->move("recursos_documentos_verificados", $name_documento);

                    $documentos->nombre_archivo = $name_documento;
                    $documentos->nombre_archivo_real = $imagen->getClientOriginalName();
                    //$documentos[] = $name_documento;
                    array_push($archivos_cargados, $name_documento);
                    $documentos->descripcion_archivo = $tipo_documento_descripcion;
                    $documentos->requerimiento = $req_id;
                    $documentos->tipo_documento_id = $tipo_documento;

                    $documentos->save();
                }else{
                    $documentos->fill($data->all() + ["tipo_documento_id"=>$tipo_documento,"user_id" =>$candidato->user_id ,"requerimiento"=>$req_id,"numero_id"=>$candidato->numero_id,"gestiono"=>$this->user->id]);
                    $documentos->save();

                    $name_documento = "documento_examenes_medicos_" . $documentos->id . "." . $extension;
                    $imagen->move("recursos_documentos_verificados", $name_documento);
                    $documentos->nombre_archivo = $name_documento;
                    $documentos->nombre_archivo_real = $imagen->getClientOriginalName();
                    $documentos->descripcion_archivo= $tipo_documento_descripcion;//"EXAMENES MÉDICOS";
        ...
        if ($salud_ocupacional == 'si') {
            //se guarda arreglo en ordenMedica
            $orden->documento = json_encode($archivos_cargados);
            $orden->save();

            //OrdenMedicaResultado
            $NuevoResultado->documento = json_encode($archivos_cargados);
            $NuevoResultado->save();
        }

        return $cant_no_procesados;
    }
}
