...
class RequerimientoController extends Controller
{
    ...
    public function nuevo_requerimiento_admin($cliente_id, $negocio_id, Request $data)
    {
        ...
        if ( in_array($cliente->nit, ['800020719', '800155500']) ) {
            $tipo_contrato = ["" => "Seleccionar"] + TipoContrato::where("active", 1)->orderBy("descripcion")->pluck("descripcion", "id")->toArray();
        }else{
            $tipo_contrato = ["" => "Seleccionar"] + TipoContrato::where("active", 1)->where("cod_tipo_cont", '<>', "03")->orderBy("descripcion")->pluck("descripcion", "id")->toArray();
        }
        ...
        if(route("home") == "http://tiempos.t3rsc.co" || route("home") == "https://tiempos.t3rsc.co"){
            ...
            $date            = Carbon::now();
            $fecha_hoy       = $date->toDateString();
            $fecha_tentativa = $date->addWeekdays(5)->toDateString();
        ...
        $nivel_5 = ["" => "Seleccionar"] + Nivel5::join("clientes_nivel_5", "clientes_nivel_5.nivel_5_id", "=", "nivel_5.id")
                                                ->where("clientes_nivel_5.cliente_id", $cliente_id)
        ...
        $nivel_6 = ["" => "Seleccionar"] + Nivel6::join("clientes_nivel_6", "clientes_nivel_6.nivel_6_id", "=", "nivel_6.id")
                                                ->where("clientes_nivel_6.cliente_id", $cliente_id)
        ...
        $nivel_7 = ["" => "Seleccionar"] + Nivel7::join("clientes_nivel_7", "clientes_nivel_7.nivel_7_id", "=", "nivel_7.id")
                                                ->where("clientes_nivel_7.cliente_id", $cliente_id)
        ...
        $nivel_8 = ["" => "Seleccionar"] + Nivel8::join("clientes_nivel_8", "clientes_nivel_8.nivel_8_id", "=", "nivel_8.id")
                                                ->where("clientes_nivel_8.cliente_id", $cliente_id)
    ...
    {   
        ...
        if(route('home') != "https://komatsu.t3rsc.co"){
            ...
            //iniciamos codigo de correo de asignado a
            $asignado_a = ($data->asignado_a != null && $data->asignado_a != '' ? $data->asignado_a : null);

            $psicologo = null;

            if ($asignado_a != null) {
                //enviar Email de asignacion de Requerimiento (asignado_a es un id de usuario)
                $psicologo = User::join("datos_basicos", "datos_basicos.user_id", "=", "users.id")
                ->select("users.email", "datos_basicos.nombres")
                ->find($asignado_a);
            }
            ...
            if ($psicologo != null) {
                ...
                $mailTitle = "Requerimiento asignado"; //Titulo o tema del correo
                //Arreglo para el botón
                $mailButton = ['buttonText' => 'IR A GESTIONAR', 'buttonRoute' => route("admin.gestion_requerimiento", [$nuevoRequerimiento->id])];

                //Cuerpo con html y comillas dobles para las variables
                $mailBody = "
                            Hola {$psicologo->nombres} te informamos que se te ha asignado el requerimiento No. {$nuevoRequerimiento->id} del cliente {$nuevoRequerimiento->nombre_cliente_req()} para el cargo {$cargo_especifico->descripcion}, haz clic en el siguiente botón para realizar la gestión correspondiente.
                ";
                

                $triEmail = triPostmaster::makeEmail($mailTemplate, $mailConfiguration, $mailTitle, $mailBody, $mailButton);

                Mail::send($triEmail->view, ['data' => $triEmail->data], function($message) use ($psicologo, $nuevoRequerimiento) {

                        $message->to($psicologo->email)
                            ->subject("Asignación de requerimiento No. {$nuevoRequerimiento->id}")
                            ->getHeaders()
                            ->addTextHeader('X-SES-CONFIGURATION-SET', 'watch_emails_t3rs');
                    });
            }

            //fin de codigo para correo asignado a
        ...
        if($data->get('select_multi_reque') != '' && $data->get('select_multi_reque') == 1){
            ...
            $asignadoARequeMulti   = $data->get('asignado_a_multi'); 
            ...
            for ($i = 0; $i < count($ciudadRequeMulti); $i++) {
                ...
                if (route('home') != "http://komatsu.t3rsc.co") {
                    ...
                    $nuevoRequerimiento->asignado_a            = $asignadoARequeMulti[$i];
                ...
                //iniciamos codigo de correo de asignado a
                $asignado_a = ($asignadoARequeMulti[$i] != null && $asignadoARequeMulti[$i] != '' ? $asignadoARequeMulti[$i] : null);
    ...
    public function guardar_requerimiento(Request $data)
    {
        ...
        //fin codigo para correo asignado a
        if($data->hasFile('perfil')) {
            $imagen     = $data->file("perfil");
    ...
    public function editar_requerimiento($requerimiento_id, Request $data)
    {
        if(route("home") == "http://komatsu.t3rsc.co" || route("home") == "https://komatsu.t3rsc.co"){
            ...
            $centro_costo = ["" => "Seleccionar"]+CentroCostoProduccion::where("estado", 'ACT')->where('active', 1)->pluck("descripcion", "id")->toArray();
            ...
            $nivel_5 = ["" => "Seleccionar"] + Nivel5::join("clientes_nivel_5", "clientes_nivel_5.nivel_5_id", "=", "nivel_5.id")
                                                    ->where("clientes_nivel_5.cliente_id", $negocio->cliente_id)
                                                    ->pluck("nivel_5.descripcion", "nivel_5.id")
                                                    ->toArray();
                
            $nivel_6 = ["" => "Seleccionar"] + Nivel6::join("clientes_nivel_6", "clientes_nivel_6.nivel_6_id", "=", "nivel_6.id")
                                                    ->where("clientes_nivel_6.cliente_id", $negocio->cliente_id)
                                                    ->pluck("nivel_6.descripcion", "nivel_6.id")
                                                    ->toArray();

            $nivel_7 = ["" => "Seleccionar"] + Nivel7::join("clientes_nivel_7", "clientes_nivel_7.nivel_7_id", "=", "nivel_7.id")
                                                    ->where("clientes_nivel_7.cliente_id", $negocio->cliente_id)
                                                    ->pluck("nivel_7.descripcion", "nivel_7.id")
                                                    ->toArray();

            $nivel_8 = ["" => "Seleccionar"] + Nivel8::join("clientes_nivel_8", "clientes_nivel_8.nivel_8_id", "=", "nivel_8.id")
                                                    ->where("clientes_nivel_8.cliente_id", $negocio->cliente_id)
                                                    ->pluck("nivel_8.descripcion", "nivel_8.id")
                                                    ->toArray();
    ...
    public function actualizar_requerimiento(Request $data)
    {
                ...
                $requerimiento->asignado_a      = $data->get('asignado_a');
                ...
                $requerimiento->nivel_5_id      = $data->nivel_5_id;
                $requerimiento->nivel_6_id      = $data->nivel_6_id;
                $requerimiento->nivel_7_id      = $data->nivel_7_id;
                $requerimiento->nivel_8_id      = $data->nivel_8_id;
                ...
                $requerimiento->auxilio_transporte   = $data->get('auxilio_transporte');
                if(route('home')!="http://komatsu.t3rsc.co") {
    ...
    public function clonar_requerimiento_view(Request $data)
    {
        ...
        ->join('clientes','clientes.id','=','users_x_clientes.cliente_id')
        ->where('role_users.role_id',17)
        ->where('clientes.id', $negocio->cliente_id)
        ->groupBy("users.id")
        ->pluck('users.name','users.id')
        ->toArray();
        ...
        return view("admin.requerimientos.modal_clonar_requerimiento_new", compact("atributos_textbox", "candidatos_postulados", "cliente", "user", "negocio", "requerimiento", "centro_costo","nombre","ciudadesSelect","fecha_tentativa","fecha_r_tentativa","fecha_hoy", "psicologos"));
    }
    ...
    public function guardar_requerimiento_copia(Request $data){
        ...
        $asignadoReqs = $data->get('asignado_a');
        ...
        for ($i = 0; $i < count($ciudadReqs); $i++) {
            ...
            if (route('home') != "http://komatsu.t3rsc.co") {
                ...
                Éxitos en tu aplicación y esperamos que este cargo aporte en tus objetivos laborales y de vida!");
                $nuevoRequerimiento->empresa_contrata              = $copia->empresa_contrata; 
                ...
                $nuevoRequerimiento->asignado_a                    = $asignadoReqs[$i];
                $nuevoRequerimiento->nivel_5_id                    = $copia->nivel_5_id;
                $nuevoRequerimiento->nivel_6_id                    = $copia->nivel_6_id;
                $nuevoRequerimiento->nivel_7_id                    = $copia->nivel_7_id;
                $nuevoRequerimiento->nivel_8_id                    = $copia->nivel_8_id;
