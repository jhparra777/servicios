...
use App\Models\PruebaCompetenciaConfigDefault;
...
class PruebaCompetenciasController extends Controller
{
    ...
    public function start(Request $data)
    {
        ...
        if (count($configuracionPrueba) <= 0) {
            $requerimiento = Requerimiento::where('id', $requerimientoId)->select('cargo_especifico_id', 'cargo_generico_id')->first();
            $configuracionPrueba = PruebaCompetenciaCargo::where('cargo_id', $requerimiento->cargo_especifico_id)->get();

            if (count($configuracionPrueba) <= 0) {
                $configDefault = PruebaCompetenciaConfigDefault::where('cargo_generico_id', $requerimiento->cargo_generico_id)->get();

                foreach ($configDefault as $configDef) {
                    $nuevaConfig = new PruebaCompetenciaReq();
                    $nuevaConfig->fill([
                        'req_id'                => $requerimientoId,
                        'competencia_id'        => $configDef->competencia_id,
                        'nivel_cargo'           => $configDef->nivel_cargo,
                        'nivel_esperado'        => $configDef->nivel_esperado,
                        'margen_acertividad'    => $configDef->margen_acertividad
                    ]);
                    $nuevaConfig->save();
                }
    ...
    public function store(Request $data)
    {
        ...
        */
        ...
        if ($sitioModulo->proceso_automatico == 'enabled') {
            $requerimiento = Requerimiento::select('solicitado_por', 'tipo_proceso_id')->find($requerimientoId);
            if ($sitioModulo->id_proceso_automatico == $requerimiento->tipo_proceso_id) {
                /**
                 * Enviar Proceso de Entrevista Virtual
                 */
                $candidato_req = ReqCandidato::where('requerimiento_id', $requerimientoId)->where('candidato_id', $userId)->first();

                $datos_candidato = ReqCandidato::join('datos_basicos', 'datos_basicos.user_id', '=', 'requerimiento_cantidato.candidato_id')
                ->where('requerimiento_cantidato.id', $candidato_req->id)
                ->select(
                ...
                $titulo  = "Video Entrevista Virtual";

                $campos_data = [
                    'requerimiento_candidato_id' => $candidato_req->id,
                ...
                $mensaje_wpp = $sitio->mensajePruebasWhatsapp($datos_candidato->nombre_completo, $titulo, route('home.video_entrevista_virtual',['user_id'=>$userId,'req_id'=>$requerimientoId]));
                ...
                $urls = route('home.video_entrevista_virtual',['user_id'=>$userId,'req_id'=>$requerimientoId]);

                $asunto = "Invitación a video entrevista de oferta de empleo";
                $emails = $datos_candidato->email;
                ...
                //Arreglo para el botón
                $mailButton = ['buttonText' => 'REALIZAR VIDEO ENTREVISTA', 'buttonRoute' => $urls];
                ...
                Mail::send($triEmail->view, ['data' => $triEmail->data], function($message) use($data, $emails, $asunto, $sitio) {
                    $message->to($emails, "T3RS")
                        ->bcc($sitio->email_replica)
                        ->subject($asunto)
                        ->getHeaders()
                        ->addTextHeader('X-SES-CONFIGURATION-SET', 'watch_emails_t3rs');
                ...
                /**
                 * Fin envio video entrevista virtual
                 */
            }
        }
