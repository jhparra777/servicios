...
use App\Models\PruebaValoresConfigDefault;
...
class PruebaValoresController extends Controller
{
    public function index_prueba(Request $data)
    {
        ...
        if (empty($configuracion)) {
            ...
            $config_default = PruebaValoresConfigDefault::where('cargo_generico_id', $requerimiento->cargo_generico_id)->orderBy('id', 'desc')->first();

            if (!empty($config_default)) {
                $configuracion = new PruebaValoresConfigRequerimiento();
                $configuracion->fill([
                    'req_id'            => $req_id,
                    'valor_amor'        => $config_default->valor_amor,
                    'valor_paz'         => $config_default->valor_paz,
                    'valor_rectitud'    => $config_default->valor_rectitud,
                    'valor_verdad'      => $config_default->valor_verdad,
                    'valor_no_violencia'   => $config_default->valor_no_violencia,
                    'tiempo_maximo'     => $config_default->tiempo_maximo,
                    'gestiono'          => $requerimiento->solicitado_por,
                    'prueba_valores_1'  => 'enabled'
                ]);
                $configuracion->save();
            } else {
                logger('No se consiguió configuración para la Prueba Ethical Values [PruebaValoresController@index_prueba]');
                return redirect()->route('dashboard')->with('no_prueba', 'Ocurrio un problema al buscar los datos.');
            }
        }
    ...
    public function save_result_valores(Request $request)
    {
        ...
        if ($sitioModulo->proceso_automatico == 'enabled') {
            ...
            if ($sitioModulo->id_proceso_automatico == $requerimiento->tipo_proceso_id) {
                ...
                ->where('requerimiento_cantidato.id', $proceso->requerimiento_candidato_id)
                ->select(
                ...
                $result_test->fill([
                    ...
                    'gestion_id'     => $requerimiento->solicitado_por
                ]);
                $result_test->save();
                ...
                $campos_data = [
                    'requerimiento_candidato_id' => $proceso->requerimiento_candidato_id,
                ...
                $mensaje_wpp = $sitio->mensajePruebasWhatsapp($datos_candidato->nombre_completo, $titulo, route('cv.competencias_inicio'));
                ...
                */
                $mailTemplate = 2; //Plantilla con botón e imagen de fondo
                $mailConfiguration = 1; //Id de la configuración
                $mailTitle = "Prueba de Competencias"; //Titulo o tema del correo

                //Cuerpo con html y comillas dobles para las variables
                $mailBody = "
                    Hola $datos_candidato->nombre_completo, has sido enviado/a en tu proceso de selección a realizar nuestra Prueba de Competencias. <br>
                ...
                Mail::send($triEmail->view, ['data' => $triEmail->data], function ($message) use ($datos_candidato, $sitio) {
                    ...
                    ->bcc($sitio->email_replica)
                    ->subject("Prueba de Competencias")
                    ->getHeaders()
                    ->addTextHeader('X-SES-CONFIGURATION-SET', 'watch_emails_t3rs');
                });
            }
