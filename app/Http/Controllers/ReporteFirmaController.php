    ...
    private function getDataDetalle($request)
    {
        ...
        if($fecha_inicio != '' || $fecha_final != '' || $cliente_id != '' || $area1 != '' ||
            ...
            ->select(
                ...
                DB::raw('(SELECT usuario_envio FROM procesos_candidato_req AS pcr WHERE pcr.proceso = \'PRE_CONTRATAR\' AND pcr.requerimiento_id = requerimientos.id AND pcr.candidato_id = datos_basicos.user_id order by pcr.created_at DESC limit 1 ) AS id_envio_precontratar'),
