        ...
        if($data != "vacio"){
            ...
            foreach ($data as &$candidato) {
                ...
                $doc_candidato = Documentos::where('user_id', $candidato->user_id)
                    ...
                    ->select("nombre_archivo", "tipo_documento_id", "descripcion_archivo")
                    ->orderBy('created_at', 'desc')
                    ->get();
                $candidato["documentos"] = collect([]);
                $candidato["porcentaje"] = 0;
...
//dd($data->toSql());
    ...
    private function getDataDetalleReporteContratados($request)
    {
       ...
       $data="vacio";
        ...
        // Data
        /*
        if($fecha_inicio!="" || $fecha_final!="" ||  $cliente_id!="" || $usuario_envio!="" ||  $req_id!=""){

            $data = FirmaContratos::leftjoin("requerimientos", "requerimientos.id", "=", "firma_contratos.req_id")
            ...
            ->where(function ($sql) use ($fecha_inicio, $fecha_final, $cliente_id, $req_id, $usuario_envio){
                if ($fecha_inicio != "" && $fecha_final != "") {
                ...
                if($usuario_envio != ""){
                    $sql->whereRaw("? = (SELECT usuario_envio FROM procesos_candidato_req where proceso in('ENVIO_CONTRATACION') AND
                     requerimiento_id=requerimientos.id AND candidato_id=datos_basicos.user_id ORDER BY procesos_candidato_req.id DESC LIMIT 1)", [$usuario_envio]);
                }
            })
            ->where("firma_contratos.estado", 1)
            ->where("firma_contratos.terminado", 1)
            ->whereRaw('requerimiento_contrato_candidato.id=(select max(requerimiento_contrato_candidato.id) from requerimiento_contrato_candidato where requerimiento_contrato_candidato.candidato_id=firma_contratos.user_id AND requerimiento_contrato_candidato.requerimiento_id=firma_contratos.req_id)')
            ->whereRaw('firma_contratos.id=(select max(firma_contratos.id) from firma_contratos where requerimiento_contrato_candidato.candidato_id=firma_contratos.user_id)')
            ->where(function ($query) {
                $ids_clientes_prueba = [];
            ...
            ->select(
                ...
                "requerimiento_contrato_candidato.fecha_fin_contrato as fecha_retiro",
                "cargos_especificos.descripcion as cargo",
                "tipos_contratos.descripcion as tipo_contrato",
                "tipos_jornadas.descripcion as tipo_jornada",
                "entidades_eps.descripcion as eps",
                "entidades_afp.descripcion as afp",
                "caja_compensacion.descripcion as caja",
                "fondo_cesantias.descripcion as fondo_cesantia",
                "requerimientos.ctra_x_clt_codigo as tarifa_arl",
                DB::raw("(SELECT ciudad.nombre FROM ciudad WHERE ciudad.cod_ciudad = requerimientos.ciudad_id 
                    AND ciudad.cod_departamento = requerimientos.departamento_id AND ciudad.cod_pais = requerimientos.pais_id LIMIT 1)
                     as centro_trabajo"),
                "centros_costos_produccion.descripcion as centro_costo",
                "nivel_5.descripcion as nivel_5",
                "nivel_6.descripcion as nivel_6",
                "nivel_7.descripcion as nivel_7",
                "nivel_8.descripcion as nivel_8",
                "bancos.nombre_banco as banco",
                "requerimiento_contrato_candidato.numero_cuenta",
                "formas_pago.descripcion as tipo_pago",
                "tipos_salarios.descripcion as forma_salario",
                "tipos_contratos.modelo_contrato as modelo_contrato",
        ...
        elseif(route("home")=="https://gpc.t3rsc.co"){
            ...
            if ($sitio->multiple_empresa_contrato && $sitio->agencias) {
                $headers = [
                    ...
                    'ind_cumplimiento_ANS',
                    'ind_calidad_proceso',
                    'Fecha ingreso ultimo contratado',
                    'Observaciones'
                ];
         ...
         if(route("home")!="http://komatsu.t3rsc.co" && route("home")!="https://komatsu.t3rsc.co"){
            ...
            //-------------------------------------------------------------------------------------
                   ...
                   /*DB::raw('(select upper(p.name)  
                    ...
                    order by o.created_at desc limit 1) as usuario_gestiono_req'),*/
                    DB::raw('(SELECT upper(u.name)
                        FROM requerimientos AS r
                        INNER JOIN users AS u ON u.id = r.asignado_a 
                        WHERE r.id = requerimiento_id) AS usuario_gestiono_req'),
...
//-------------------------------------------------------------------------------------
                ...
                where ( estado in('.config('conf_aplicacion.C_EN_PROCESO_CONTRATACION').') and requerimiento_id=requerimientos.id) and ( cand_presentado_puntual=1 and cand_contratado=1 )
                group by requerimiento_id)/requerimientos.num_vacantes)*100) as ind_calidad_presentacion'),
                DB::raw('(select fecha_ingreso from requerimiento_contrato_candidato where requerimiento_id=requerimientos.id order by created_at desc limit 1 ) as fecha_ultimo_contratado')
                )
                ->groupBy('requerimientos.id')
                ->orderBy('requerimiento_id','desc');
//dd($data->toSql());
...
//dd($data);
        Excel::create('reporte-excel-contratados', function ($excel) use ($data, $headers, $formato) {
            $excel->setTitle('Reporte Contratados');
            $excel->setCreator('$nombre')
                ->setCompany('$nombre');
            $excel->setDescription('Reporte Contratados');
