...
class ContratacionVirtualController extends Controller {
    ...
    */
    public function firmaContratoCandidato($user_id, $req_id, $modulo = 'modulo_cv')
    {
        ...
        ->leftJoin("formas_pago", "formas_pago.id", "=", "requerimiento_contrato_candidato.forma_pago_id")
        ->leftJoin("caja_compensacion", "caja_compensacion.id", "=", "requerimiento_contrato_candidato.caja_compensacion_id")
        ->where('requerimiento_contrato_candidato.requerimiento_id', $ReqId)
        ->where('requerimiento_contrato_candidato.candidato_id', $userId)
        ->select(
            ...
            'bancos.nombre_banco',
            'centros_costos_produccion.descripcion as centro_costo_produccion',
            'formas_pago.descripcion as forma_pago',
    ...
    */
    public function confirmar_documentos_adicionales($user_id, $contrato_id, $moduloHash)
    {
        ...
        */
        ...
        $adicional_medico = false;  //Para controlar si lleva clausula medica
        $especificaciones = null;   //En caso que lleve la clausula medica: si es null es porque ya firmo la clausula - si es distinto de null es porque no ha firmado la clausula
        $requerimiento_candidato_orden = null;
        ...
        if ($sitio_modulo->clausula_medica == 'enabled') {
            if ($sitio_modulo->salud_ocupacional == 'si') {
                ...
                $requerimiento_candidato_orden = OrdenMedica::leftJoin("proveedor", "proveedor.id", "=", "orden_medica.proveedor_id")
                ->where('orden_medica.req_can_id', $requerimiento_candidato->id)
                ->select(
                        "orden_medica.*",
        ...
        */
        ...
        if ($documento == null || count($documentos_parametrizados) <= 0) {
            ...
            if ($firmaContratoFin->terminado != 3) {
                ...
                ->leftJoin("formas_pago", "formas_pago.id", "=", "requerimiento_contrato_candidato.forma_pago_id")
                ->leftJoin("caja_compensacion", "caja_compensacion.id", "=", "requerimiento_contrato_candidato.caja_compensacion_id")
                ->where('requerimiento_contrato_candidato.requerimiento_id', $req_id)
                ->where('requerimiento_contrato_candidato.candidato_id', $userId)
                ->select(
                    'requerimiento_contrato_candidato.fecha_ingreso',
                    'requerimiento_contrato_candidato.fecha_fin_contrato',
                ...
                if ($sitio_modulo->clausula_medica == 'enabled') {
                    if ($sitio_modulo->salud_ocupacional == 'si') {
                        ...
                        //Cláusula medica
                        $adicional_externo = ConfirmacionDocumentosAdicionales::where("user_id", $this->user->id)
                        ->where("contrato_id", $contratoId)
                        ->where("externo", 2)
                        ->orderBy('created_at', 'DESC')
                        ->first();
    ...
    */
    public function guardarFirmaAdicional(Request $data)
    {
        ...
        if($data->has('recomendaciones')){
            ...
            $requerimiento = Requerimiento::leftJoin('empresa_logos', "empresa_logos.id", "=", "requerimientos.empresa_contrata")
            ->where('requerimientos.id', $firma_contrato->req_id)
            ->select(
                    "requerimientos.*",
                    "empresa_logos.nombre_empresa",
                    "empresa_logos.logo")->first();
            ...
            if($data->has('creada')) {
                ...
                $empresa_contrata = EmpresaLogo::where('id', $requerimiento->empresa_contrata)->first();
        ...
        if ($nro_pregunta == $cantidad_preguntas) {
            ...
            if ($terminadoAnterior == null) {
                ...
                $fechasContrato = RequerimientoContratoCandidato::leftJoin("entidades_afp", "entidades_afp.id", "=", "requerimiento_contrato_candidato.fondo_pensiones_id")
                    ...
                    ->leftJoin("caja_compensacion", "caja_compensacion.id", "=", "requerimiento_contrato_candidato.caja_compensacion_id")
                    ->where('requerimiento_contrato_candidato.requerimiento_id', $ReqId)
                    ->where('requerimiento_contrato_candidato.candidato_id', $user_id)
                    ->select(
                        'requerimiento_contrato_candidato.fecha_ingreso',
                        'requerimiento_contrato_candidato.fecha_fin_contrato',
                        'requerimiento_contrato_candidato.salario',
            ...
            /*$view3 = \SnappyPDF::loadView('home.pdf-carta-presentacion', [
                'candidato'     => $candidato,
                'reqcandidato'  => $reqcandidato,
            ...
            $view3 = \View::make('home.pdf-carta-presentacion', [
                'candidato'     => $candidato,
                'reqcandidato'  => $reqcandidato,
                'empresa_logo'  => $empresa_logo,
                'req_contrato_candidato' => $req_contrato_candidato
            ...
            Storage::disk('public')->put('recursos_carta_presentacion/'.$nombre_documento,$output_carta);
            ...
            $pdf_carnet =  app('dompdf.wrapper');
            $pdf_carnet->loadHTML(view("home.carnet_candidato_general", [
                'candidato' => $candidato,
                'req' => $req_carnet,
            ...
            $output_carnet = $pdf_carnet->output();
            //fin de carnet
            ...
            //Cuerpo con html y comillas dobles para las variables
            $mailBody = "
                Felicitaciones {$candidato->primer_nombre} {$candidato->segundo_nombre} {$candidato->primer_apellido} {$candidato->segundo_apellido},  has finalizado la firma digital de tu contrato correspondiente al cargo {$reqcandidato->nombre_cargo_especifico} en {$reqcandidato->cliente_nombre} <br/><br/>

                Adjunto encontrarás tu carnét y la carta que debes presentar al momento de ingresar a laborar.";
            ...
            Mail::send($triEmail->view, ['data' => $triEmail->data], function($message) use ($ReqId, $candidato, $output_carta, $output_carnet) {

                    $message->to([$candidato->email], "Proservis")
                    ->subject("Bienvenido/a a Proservis 👏");
                    if ($output_carta != null) {
                        $message->attachData($output_carta,'carta_presentacion.pdf');
                    }
                    if ($output_carnet != null) {
                        $message->attachData($output_carnet,'carnet.pdf');
                    }
                    $message->getHeaders()
                    ->addTextHeader('X-SES-CONFIGURATION-SET', 'watch_emails_t3rs');
                });
        ...
        //Si es la ultima pregunta, actualiza registro de firma
            ...
            if ($terminadoAnterior == null) {
                ...
                $fechasContrato = RequerimientoContratoCandidato::leftJoin("entidades_afp", "entidades_afp.id", "=", "requerimiento_contrato_candidato.fondo_pensiones_id")
                    ...
                    ->leftJoin("caja_compensacion", "caja_compensacion.id", "=", "requerimiento_contrato_candidato.caja_compensacion_id")
                    ->where('requerimiento_contrato_candidato.requerimiento_id', $ReqId)
                    ->where('requerimiento_contrato_candidato.candidato_id', $this->user->id)
                    ->select(
                        'requerimiento_contrato_candidato.fecha_ingreso',
                        'requerimiento_contrato_candidato.fecha_fin_contrato',
                        'requerimiento_contrato_candidato.salario',
