...
class ContratacionController extends Controller
{
    ...
    */
    public function index(Request $data)
    {
        ...
        ->where(function ($sql) use ($data, &$procesos, &$perro, &$estado, &$estados_requerimiento, &$ordenamiento, $sitio) {
            ...
            if ($data->has("agencias") && $data->get("agencias") != "") {
                $sql->where("ciudad.agencia", $data->get("agencias"));
            }else{
                $sql->whereIn("ciudad.agencia", $this->user->agencias());
            }

            if (count($data->all()) == 0) {
                $sql->whereNotIn('requerimiento_cantidato.estado_candidato', [config('conf_aplicacion.C_CONTRATACION_CANCELADA'),config('conf_aplicacion.C_PROCESO_CANCELADO')]);
            }
        })
    ...
    */
    public function documentos_seleccion($candidato, $req, $req_can)
    {
        ...
        if($req_candidato->bloqueo_carpeta){
            ...
            $docs = Documentos::select('nombre_archivo as nombre','nombre_archivo_real as nombre_real', 'id as id_documento', 'fecha_vencimiento as fecha_vencimiento','tipo_documento_id')->where('user_id', $candidato_id)->where("active",1)->get();
    ...
    */
    public function documentos_contratacion($candidato, $req)
    {
        ...
        $docs = Documentos::select('nombre_archivo as nombre','nombre_archivo_real as nombre_real', 'id as id_documento','tipo_documento_id')->where('user_id', $candidato_id)->where('requerimiento', $req)->where("active",1)->latest('id')->get();
        ...
        $carta_presentacion = Documentos::where("requerimiento", $req)
        ->where("user_id",$candidato )
        ->where("nombre_archivo", 'like', 'carta_presentacion_' . '%')
        ->select("nombre_archivo", "id")
        ->orderBy('id', 'DESC')
        ->first(); 
    ...
    */
    public function documentos_confidenciales($candidato, $req)
    {
        ...
        foreach ($tipo_documento as $key => &$tipo_doc) {
            $tipo_doc->documentos = DB::table('documentos')->select('nombre_archivo as nombre', 'id as id_documento', 'nombre_archivo_real')->where('user_id', $candidato_id)->where('tipo_documento_id', $tipo_doc->id)->where('requerimiento', $req)->latest('id')->limit(5)->get();
        }
        unset($tipo_doc);
    ...
    public function documentos_post($candidato, $req)
    {
        ...
        /*foreach ($tipo_documento as $key => &$tipo_doc) {
            $tipo_doc->documentos = DB::table('documentos')->select('nombre_archivo as nombre', 'id as id_documento')->where('user_id', $candidato_id)->where('tipo_documento_id', $tipo_doc->id)->where('requerimiento', $req)->latest('id')->limit(5)->get();
        }*/
    ...
    public function documentos_beneficiarios($candidato, $req) {
        ...
        $documentosFamiliares = DocumentoFamiliar::leftjoin("grupos_familiares", "grupos_familiares.id", "=", "documentos_familiares.grupo_familiar_id")
            ...
            ->leftjoin("tipo_afiliacion", "tipo_afiliacion.id", "=", "documentos_familiares.tipo_afiliacion_id")
            ...
            ->select("documentos_familiares.id",
                     ...
                     "tipo_afiliacion.descripcion as tipo_afiliacion")
    ...
    public function cargar_documento_admin_beneficiario(Request $request)
    {
        ...
        $tipos_afiliacion = ["" => "Seleccionar"] +  DB::table("tipo_afiliacion")->where("active", "1")->pluck("descripcion", "id")->toArray();
        ...
        return view("admin.contratacion.candidato.modal.nuevo_documento_beneficiario_asistente", compact("tipos_documentos", "gruposFamiliares", "cand_id", "tipos_afiliacion"));
    }
    ...
    public function cargar_documento_admin_contratacion(Request $request)
    {
        ...
        ->where("tipos_documentos.estado", 1)
        ->where("cargo_documento.cargo_especifico_id", $requerimiento->cargo_especifico_id)
        ->where(function($query) use ($firmaContrato) {
    ...
    public function salud_ocupacional(Request $data)
    {
        ...
        return view("admin.contratacion.lista_salud_ocupacional", compact("candidatos"));
    }
    ...
    public function ver_status_contratacion(Request $request)
    {
        ...
        if($sitioModulo->consulta_seguridad === 'enabled') {
            ...
            $consulta_seguridad_proceso = RegistroProceso::where('candidato_id', $request->get("user_id"))
            ->where('requerimiento_id', $reqCandidato[0]->requerimiento_id)
            ->where('proceso', 'CONSULTA_SEGURIDAD')
            ->first();
        }
    ...
    public function envio_documentos_contratacion(Request $data)
    {
        ...
        ->select(
            ...
            "centros_trabajo.nombre_ctra as centro_trabajo",
        ...
        if ($data->carnet != null) {
            $candidato_carnet = ReqCandidato::join("datos_basicos", "datos_basicos.user_id", "=", "requerimiento_cantidato.candidato_id")
                ...
                ->select("datos_basicos.*", "requerimiento_cantidato.id as req_candidato","requerimiento_cantidato.requerimiento_id as req","users.foto_perfil","users.avatar","procesos_candidato_req.fecha_inicio_contrato", "tipo_identificacion.cod_tipo")
            ...
            $pdf->loadHTML(view("home.carnet_candidato_general", [
        ...
        if ($data->carta_presentacion != null) {
            ...
            $req_contrato_candidato = RequerimientoContratoCandidato::leftJoin("entidades_afp", "entidades_afp.id", "=", "requerimiento_contrato_candidato.fondo_pensiones_id")
                ->leftJoin("entidades_eps", "entidades_eps.id", "=", "requerimiento_contrato_candidato.eps_id")
                ->leftJoin("caja_compensacion", "caja_compensacion.id", "=", "requerimiento_contrato_candidato.caja_compensacion_id")
                ->select(
                    "requerimiento_contrato_candidato.fecha_ingreso as fecha_inicio_contrato",
            ...
            $empresa_logo = DB::table("empresa_logos")->where('id', $reqcandidato->empresa_contrata)->first();


            $pdf_carta_presentacion =  app('dompdf.wrapper');
            $pdf_carta_presentacion->loadHTML(view('home.pdf-carta-presentacion', [
        ...
        if($data->otro_destinatario != null || $data->otro_destinatario != ''){
            ...
            Mail::send('admin.contratacion.mail.email-documentos-contratacion-cliente', [
                ...
                "cand_req"       => $cand_req
            ], function ($message) use($otro_destino, $requerimiento, $pdf, $pdf_carta_presentacion){
                if ($pdf_carta_presentacion != null) {
                    $message->attachData($pdf_carta_presentacion->output(),'carta_presentacion.pdf');
                }
                if ($pdf != null) {
        ...
        if($quien_envio_contratar->email != "" && $quien_envio_contratar->email != null){
            Mail::send('admin.contratacion.mail.email-documentos-contratacion-cliente', [
                ...
                "contrato"       => $firmaContrato,
                "cand_req"       => $cand_req
            ], function ($message) use($quien_envio_contratar, $requerimiento, $pdf, $pdf_carta_presentacion){
                if ($pdf_carta_presentacion != null) {
                    $message->attachData($pdf_carta_presentacion->output(),'carta_presentacion.pdf');
                }
                if ($pdf != null) {
        ...
        if(isset($data->candidato) && $data->candidato == 'true' && isset($user->email)) {
            ...
            Mail::send('admin.contratacion.mail.email-documentos-contratacion-cliente', [
                ...
                "cand_req"       => $cand_req,
                "empresa"        => $empresa
            ], function ($message) use($user, $requerimiento, $pdf, $pdf_carta_presentacion){
                if ($pdf_carta_presentacion != null) {
                    $message->attachData($pdf_carta_presentacion->output(),'carta_presentacion.pdf');
                }
                if ($pdf != null) {
    ...
    public function cancelar_contratacion(Request $data)
    {
        ...
        $nombre_frm = ucfirst(mb_strtolower($data->nombre));

        $motivos = ["" => "Seleccione"] + MotivosCancelacionContratacion::pluck("descripcion", "id")->toArray();
        
        return view("admin.contratacion.modal.cancelar_contratacion", compact("candidato", "req_can", "motivos", "req_id", "nombre_frm"));
    }
    ...
    public function confirmar_cancelar_contratacion(Request $data)
    {
        ...
        # Si tiene firma, se envió a contratar y se procede a cambiar el estado
        if (!empty($firma_contrato)) {
            ...
            # No tiene registro en tb firmaContrato, es un precontratado y se procede a cambiar estado a CANCELAR PROCESO

             //Cambia estado del proceso del candidato
             $aptoProceso = RegistroProceso::where('requerimiento_id', $data->req_id)
             ->where('candidato_id', $req_can->candidato_id)
             ->where('proceso', 'PRE_CONTRATAR')
             ->orderby('created_at', 'DESC')
             ->first();
             ...
             $nuevo_estado->estado_candidato = 27;
             $nuevo_estado->save();
             ...
             $nuevo_proceso->fill([
                 'requerimiento_candidato_id' => $nuevo_estado->id,
                 'estado'                     => config('conf_aplicacion.C_PROCESO_CANCELADO'),
                 'fecha_inicio'               => date("Y-m-d H:i:s"),
                 'usuario_envio'              => $this->user->id,
                 'requerimiento_id'           => $data->req_id,
                 'candidato_id'               => $req_can->candidato_id,
                 'observaciones'              => "$motivo_desc->descripcion - Cancelado desde (Módulo admin)",
                 'proceso'                    => "CANCELA_PROCESO",
                 'apto'                       => 1
             ]);
             $nuevo_proceso->save();
        }
    ...
    public function pre_contratar_view(Request $data)
    {
        ...
        $candidato = ReqCandidato::join("datos_basicos", "datos_basicos.user_id", "=", "requerimiento_cantidato.candidato_id")
            ...
            ->select(
                ...
                "requerimientos.fecha_ingreso as fecha_inicio"
            )
        ->first();

        $condicion_apto = RegistroProceso::where("requerimiento_candidato_id", $data->candidato_req)->where("proceso", "ENVIO_ENTREVISTA")->where("apto","0")->first();
/*
        if ($condicion_apto->apto=="0"){
            $candidato->observacion_no_apto = ['tipo' => 99, 'descripcion' => 'El candidato tiene un concepto NO APTO en la entrevista, no podras enviarlo al Pre-Contratar.'];
...
*/
        if ($candidato->tipo_id == null || $candidato->direccion == null || $candidato->entidad_eps == null || $candidato->entidad_afp == null ||
            ...
            if ($proceso_precontrato != null) {
                ...
                $candi_no_cumplen->push($candidato);
                $candidato = null;
            } elseif ($proceso_precontrato != null || $condicion_apto->apto=="0") {
                $candidato->observacion_no_cumple = ['tipo' => 3, 'descripcion' => 'El candidato tiene un concepto NO APTO en la entrevista, no podras enviarlo al Pre-Contratar.'];
                $candi_no_cumplen->push($candidato);
                $candidato = null; 
        ...
        $usuario_asignado = ["" => "Seleccionar"] + User::leftJoin("role_users", "role_users.user_id", "=", "users.id")
        ->where("role_users.role_id", "=", 19) // rol de contratacion
        ->select(
            "users.id",
            "users.name"
        )
        ...
        return response()->json([
            "success" => true,
            "view"   => view("admin.reclutamiento.modal.envio_pre_contratar", compact('candidato', 'candi_no_cumplen', 'usuario_asignado','condicion_apto'))->render()
        ]);
    }
    ...
    public function pre_contratar(Request $data)
    {
        ...
        $req = ReqCandidato::find($req_candi_id);
        $req->fecha_tentativa_ingreso = $data->fecha_inicio;
        $req->save();
        ...
        $usuario_asignado = DatosBasicos::select('email')->where('user_id', $data->usuario_asignado_id)->first();
        ...
        // if ($existe == false) {
            Mail::send($triEmail->view, ['data' => $triEmail->data], function($message) use($usuario_asignado, $requerimiento, $candidato) {

                $message->to($usuario_asignado->email)
        ...
        //****************enviar correo de evaluacion SST*********

        $nuevo_proceso2 = new RegistroProceso();
        $nuevo_proceso2->fill([
        ...
        $url = "realizar-evaluacion-sst+".$candidato->req_id;
        ...
        //Arreglo para el botón
        $mailButton = ['buttonText' => 'Realizar Evaluación', 'buttonRoute' => route('realizar_evaluacion_induccion_sst', ['req_id' => $candidato->req_id])];
    ...
    {   
        ...
        //$view = \View::make('cv.pdf_tratamiento_datos_personales', compact('candidato'))->render();
        $view = \View::make('home.tratamiento_datos_personales', compact('candidato'))->render();
