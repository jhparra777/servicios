    ...
    */
    ...
    public function listadoDocumentosClientes($categoria,$cliente){
        ...
        ->select("tipos_documentos_clientes.*",
            DB::raw("(select CONCAT(documentos_clientes.nombre_archivo,'|',created_at) from documentos_clientes where cliente_id=$cliente and documentos_clientes.tipo_documento_id=tipos_documentos_clientes.id and documentos_clientes.active = 1 order by documentos_clientes.id desc limit 1) as nombre"),
            ...
            DB::raw("(select d.created_at from documentos_clientes as d where d.cliente_id=$cliente and d.tipo_documento_id=tipos_documentos_clientes.id and d.active = 1 order by d.id desc limit 1) as fecha_carga"),
            DB::raw("(select u.name from documentos_clientes d left join users u on d.gestiono=u.id where d.cliente_id=$cliente and d.tipo_documento_id=tipos_documentos_clientes.id and d.active = 1 order by d.id desc limit 1) as usuario_gestiono")
        )
        ->get();
        ...
        foreach ($tipo_documento as $key => &$tipo_doc) {
            $tipo_doc->documentos = DB::table('documentos_clientes')->select('nombre_archivo as nombre','nombre_archivo_real as nombre_real', 'id as id_documento')->where('active', 1)->where('cliente_id', $cliente->id)->where('tipo_documento_id', $tipo_doc->id)->latest('id')->get();
        }
         unset($tipo_doc);
    ...
    public function guardarDocumentoCliente(Request $data){
        ...
        if($data->hasFile('archivo_documento') && !$validator->fails()){
            ...
            if(in_array($extension,$validas)) {
                ...
                $name_documento = str_replace(" ","_",$tipo_documento->descripcion)."_".date('d-m-Y_G_i_s').".". $extension; //cambiar al terminar ('d-m-Y_G:i:s')
...
;
    ...
    */
    ...
    public function downloadCarpetasGDC($cliente_id){

        //Se consultan los documentos cargados y activos del cliente
        $documentos_cliente = CategoriaDocumentoCliente::join("tipos_documentos_clientes", "tipos_documentos_clientes.categoria", "=", "categoria_documentos_clientes.id")
        ->leftjoin('documentos_clientes', 'documentos_clientes.tipo_documento_id', '=', 'tipos_documentos_clientes.id')
        ->where('documentos_clientes.active', 1)
        ->where('documentos_clientes.cliente_id', $cliente_id)
        ->select('documentos_clientes.id as id_documento',
        'documentos_clientes.nombre_archivo as nombre_archivo',
        'documentos_clientes.nombre_archivo_real as nombre_real', 
        'categoria_documentos_clientes.descripcion')
        ->orderby('categoria_documentos_clientes.id')
        ->get();

        //Si tiene algun archivo que descargar
        if (count($documentos_cliente) > 0) {

            //se crea carpeta con el nombre del cliente
            $nombre_carpeta = Clientes::where("id", $cliente_id)->first();
            $nombre_carpeta = str_replace('.', '', $nombre_carpeta->nombre);
            
            //Se crea el zip
            $zip = new ZipArchive();
            $zip->open($nombre_carpeta.".zip",ZipArchive::CREATE);

            //Agregamos los archivos existentes en el zip
            foreach ($documentos_cliente as $doc) {
                $ruta_doc = 'documentos_clientes/'.$cliente_id.'/'.$doc->descripcion."/".$doc->nombre_archivo;
                $ruta_archivo_en_zip = $doc->descripcion.'/'.$doc->nombre_archivo;
                if(file_exists(public_path($ruta_doc))){
                    $name_extention = explode(".",$tipo->nombre);
                    $zip->addFile(public_path($ruta_doc),$ruta_archivo_en_zip);
                }
            }
            ...
            return redirect()->route('admin.gestion_documental.clientes')->with("mensaje_success", "No hay documentos para descargar");
        }
    }

    public function download_categoria_carpeta_GDC($cliente_id, $categoria_id){
        ...
        ->where('documentos_clientes.active', 1)
        ->where('documentos_clientes.cliente_id', $cliente_id)
        ->where('tipos_documentos_clientes.categoria', $categoria_id)
        ->select('documentos_clientes.id as id_documento',
        'documentos_clientes.nombre_archivo as nombre_archivo',
        'documentos_clientes.nombre_archivo_real as nombre_real', 
        'categoria_documentos_clientes.descripcion')
        ...
        if (count($documentos_cliente) > 0) {
            ...
            return redirect('admin/gestionar-cliente-gestion-documental/'.$cliente_id)->with("mensaje_success", "No hay documentos para descargar");
        }
    }
}
