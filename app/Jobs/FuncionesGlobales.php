...
{   /*constantes que representan la categoria  DOCUMENTOS BENEFICIARIOS*/
    ...
    */
    ...
    public static function validaBotonPreContratar($candidato_req)
    {
        
        $consulta = RegistroProceso::where("requerimiento_candidato_id", $candidato_req)->where("proceso", 'ENVIO_ENTREVISTA')->where("apto", 0)->get();
    ...
    */
    ...
    /**
     * devuelve true si es estado del candidato es C_EN_PROCESO_CONTRATACION
     * 
     * @param candidato_req is the id of the candidate
     * @param estados array of integers
     */
    public static function validaBotonCancelarProceso($candidato_req, $estados)
    {
        $proceso = RegistroProceso::where("requerimiento_candidato_id", $candidato_req)->whereIn("estado", $estados)->get();
        if ($proceso->count() > 0) {
            //Buscar si tiene proceso anulado
            return "CANCELAR CONTRATACIÓN";
        }
        return "CANCELAR PROCESO";
    }
    ...
    /**
     * It converts all the characters to uppercase and removes the accents.
     * 
    ...
    public static function eliminar_tildes($cadena){
        ...
        //Codificamos la cadena en formato utf8 en caso de que nos de errores y se convierte en mayuscula
