...
@section("contenedor")
            ...
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <form id="fr_documento_familiar" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="tipo_documento_gf">Tipo de Afiliación: <span class='text-danger sm-text-label'>*</span></label>

                                {!! Form::select("tipo_afiliacion_id",$tipos_afiliacion,null,["class"=>"form-control | tri-br-1 tri-fs-12 tri-input--focus tri-transition-300","id"=>"tipo_afiliacion_id"]) !!}
                            </div>
                        </div>
                        ...
                        <tbody>
                            @forelse ($documentosFamiliares as $documento)
                                <tr id="tr_{{ $documento->id }}">
                                    ...
                                    <td>{{ $documento->tipo_afiliacion }}</td>
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: (response) => {
                        if (response.success) {
                            ...
                            document.querySelector('#tipo_afiliacion_id').value = ''
        ...
        const editarDocumentoGf = (event) => {
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    document.querySelector('#tipo_afiliacion_id').value = response.documento.tipo_afiliacion_id
        ...
        $(document).on("change","#tipo_documento_cv",function(){
                let tipo_documento_id=$(this).val();
                $.ajax({
                    url: "{{route('tiene_vencimiento')}}",
                    type: "post",
                    data: {
