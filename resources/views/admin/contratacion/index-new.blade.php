    ...
    {!! Form::model(Request::all(), ["route" => "admin.asistente_contratacion", "method" => "GET"]) !!}
        <div class="row">
            ...
            <div class="col-md-6 form-group">
                ...
                {!! Form::select('agencias', $agencias, null, [ 'class'=>'form-control js-example-basic-single2 | tri-br-1 tri-fs-12 tri-input--focus tri-transition-300']) !!}
                                ...
                                <td>
                                    <div class="btn-group-vertical">
                                                ...
                                                <div class="btn-group-vertical" role="group" aria-label="..." style="width: 100%;">
                                                    ...
                                                    @if($user_sesion->hasAccess("admin.contratacion.cancelar_contratacion_asistente"))
                                                        <button
                                                            ...
                                                            <?php 
                                                                $nombre_boton = App\Jobs\FuncionesGlobales::validaBotonCancelarProceso(
                                        ...
                                        @if($sitio->precontrata == 1)
                                            <?php
                                                $ultimo_proceso = $processes_collect->reverse()->first();
                                            ?>
                                            @if ($candidato->nombre_proceso == 'PRE_CONTRATAR' || $ultimo_proceso->proceso == 'CONTRATO_ANULADO' || $ultimo_proceso->proceso == 'CANCELA_CONTRATACION' || $ultimo_proceso->proceso == 'CANCELA_PROCESO')
                                                <button
                                                    class="btn btn-primary btn-sm btn-block | tri-br-2 tri-fs-12 tri-txt-purple tri-bg-white tri-bd-purple tri-transition-300 tri-hover-out-purple"
                                                    ...
                                                    @if (in_array('ENVIO_SST',$processes_apto))
                                                        {!!
                                                        ...
                                                        !!}
                                                    @else
                                                        title="Debe enviar el proceso {{$configuracion_sst->titulo_boton_envio_proceso}} y ser gestionado por el candidato"
                                                        disabled="disabled"
                                                    @endif
                                                >
                                                    CONTRATAR
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('.js-example-basic-single2').select2({
                placeholder: 'Selecciona o busca una agencia'
            });
        ...
        $(document).on("click", "#guardar_observacion", function() {
            ...
            @if($sitioModulo->evaluacion_sst == 'enabled')
                ...
                $(".btn_evaluacion_sst_masivo").on("click", function() {
                    if ($("input[name='req_candidato[]']").serialize() != "") {
                        ...
                        mensaje_success("Debe seleccionar al menos un candidato");
                    }
                });
            ...
            function validar_nro_cta() {
                //Se valida que el nro de cta sea diferente a la cedula del candidato
                var c_una =  $("#numero_cuenta").val();
                var c_dos =  $("#confirm_numero_cuenta").val();
                var nro_cedula =  $("#numero_id").val();

                if (c_una == nro_cedula || c_dos == nro_cedula) {
                    $.smkAlert({
                        text: 'El nro cta no puede ser su cédula',
                        type: 'danger',
                        time: 5
                    });
                    $("#numero_cuenta").val("")
                    $("#confirm_numero_cuenta").val("");
                    return false;
                }else{
                    return true;
                }
